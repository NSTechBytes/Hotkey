...
& $msbuildPath "Hotkey.sln" /t:Build /p:Configuration=$Configuration /p:Platform=$Platform /m /v:minimal
...
if ($LASTEXITCODE -eq 0) {
    ...
    $addonName = if ($addonMeta -and $addonMeta.name) { $addonMeta.name } else { "Hotkey" }
    ...
    $widgetsSrc = Join-Path $root "Widgets"
    $widgetsDest = $packageRoot
    $widgetsSrcRoot = Join-Path $widgetsSrc "src"
    $widgetsDestSrc = Join-Path $widgetsDest "src"
    ...
    $widgetsMeta = Join-Path $widgetsSrc "meta.json"
    if (Test-Path $widgetsMeta) {
        Copy-Item $widgetsMeta $widgetsDest -Force
    } else {
    ...
    if (-not (Test-Path $widgetsSrc)) {
        Write-Host "Warning: Widgets folder not found at $widgetsSrc" -ForegroundColor Yellow
    }
